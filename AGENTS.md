...
- **Dashboard:** build the PostHog funnel insight (`$pageview /` → `signup` → `mcp_token_created` → `agent_call`) with a breakdown on `initial_utm_source`/`initial_referrer` (PostHog-side config, not code).
...
- `0029_user_consent` — adds nullable `user.analytics_consent` ('granted'\|'denied'\|null) for cookie-less server-side analytics capture (see **Activation Funnel Analytics**). Idempotent (PRAGMA column check). Script: `src/db/apply-0029-user-consent.ts`. Apply to both local + Turso. **Next migration: `when` > `1781500000000`.**
...
- **Browser-style tabs (Tauri desktop only)** — Notion-like top tab strip. **Only rendered inside the Tauri shell** (web is unchanged — users open OS browser tabs there). Gated via `src/lib/hooks/useIsTauri.ts` (`'__TAURI_INTERNALS__' in window` check, SSR-safe).
  - `src/components/providers/TabsContext.tsx` — `TabsProvider` (client) holding **only** `tabs: Tab[]` (`{id, href, title, icon, iconColor}`; `href` normalized to `/page/<id>` · `/db/<id>` · `/db/<id>/<pageId>`). `useTabs()` returns `null` outside the provider (web no-op). **`activeId` is DERIVED from `usePathname()`** (hrefs are unique → no separate active state to drift). A matching tab activates; otherwise the tab that matched the *previous* path **navigates in place** (browser model) — single sidebar click follows the active tab, no duplicate. The pathname-reconcile effect uses a **functional `setTabs` updater** so rapid new-tab opens never clobber each other (fixes the earlier "tabs vanish" bug). `openInNewTab(href, metaHint?)` appends a placeholder tab (`pendingNewTabRef`) then `router.push`; the next navigation resolves the placeholder's final href (so `+` → `/app` collapses into the redirect target). Meta for `/page` & `/db` tabs resolves **live** from the `items` prop (renames reflect); DB-row (`/db/x/y`) titles come from `document.title` via a `<title>` `MutationObserver`. A **global capture-phase `auxclick`/`click` listener** intercepts middle-click and Ctrl/Cmd+click on any internal `<a href>` (sidebar, in-editor page links, modal links) → `openInNewTab`. Persisted to `localStorage['remnus_tabs_<workspaceId>']` (per workspace; deleted items pruned on load). Provider **keyed by `activeWorkspaceId`** in `AppShell` (each workspace its own tab set).
