...
- `BlockSelectionExtension` (`BlockSelectionExtension.ts`) — ProseMirror plugin for Notion-style multi-block selection. State: `{ selected: number[], anchor: number | null }` keyed by `blockSelectionKey`. Drag from any block to select a range (cross-block-boundary detection via 6px threshold); shift+click extends the selection; `ArrowUp`/`ArrowDown` moves the head; `Delete`/`Backspace` deletes all selected blocks; `Escape` clears. Visual blocks: top-level non-list nodes + individual `listItem`/`taskItem` nodes. Applies `Decoration.node(..., { class: 'block-selected' })` for each selected position. Also adds `.block-selecting` to the editor DOM while dragging (suppresses `user-select`).
- `BlockSelectionToolbar` (`BlockSelectionToolbar.tsx`) — Floating toolbar rendered when `BlockSelection` has blocks selected. Shows count (`blockSelectionCount` i18n), Duplicate, and Delete buttons. Positioned above the first selected block using `view.coordsAtPos`. Cleared after each action.
