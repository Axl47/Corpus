...
function buildDecorations(doc: PMNode, collapsed: Set<number>): DecorationSet {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    for (let j = i + 1; j < children.length; j++) {
      const s = children[j];
      if (
        s.node.type.name === "heading" &&
        (s.node.attrs.level as number) <= level
      )
        break;
      decos.push(
        Decoration.node(s.offset, s.offset + s.node.nodeSize, {
          class: "heading-collapsed-child",
        }),
      );
    }
  }
...
export const CollapsibleHeading = Heading.configure({
  levels: [1, 2, 3],
}).extend({
  addAttributes() {
    return {
      ...
      new Plugin({
        ...
        state: {
          init(config, state) {
            ...
            if (pageId && typeof window !== "undefined") {
              try {
                const saved = localStorage.getItem(
                  `corpus_collapsed_headings_${pageId}`,
                );
                if (saved) {
                ...
                console.error(
                  "Error loading collapsed headings from localStorage:",
                  e,
                );
              }
            }
            return {
              collapsed,
              decorations: buildDecorations(state.doc, collapsed),
            };
          },
          apply(tr, prev) {
            ...
            const meta = tr.getMeta(headingCollapseKey) as
              | { toggle?: number }
              | undefined;
            ...
            if (pageId && typeof window !== "undefined") {
              try {
                ...
                console.error(
                  "Error saving collapsed headings to localStorage:",
                  e,
                );
              }
            }
            ...
            return {
              collapsed: mapped,
              decorations: buildDecorations(tr.doc, mapped),
            };
          },
        },
        props: {
