...
} from "@/components/features/editor/BlockEditor";
...
export default function PageEditor({
  ...
  useEffect(() => {
    if (!isPeek) {
      document.title = `${pageTitle} | Corpus`;
    }
  }, [pageTitle, isPeek]);
  ...
  const handleMultiSelectToggle = async (colId: string, option: string) => {
    const current: string[] = Array.isArray(properties[colId])
      ? properties[colId]
      : [];
        ...
        {schema
          ...
          .map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`flex items-start gap-2 border-b border-neutral-800/60 group ${isPeek ? "flex-row gap-3 pb-1.5" : "flex-col sm:flex-row sm:gap-8 pb-3"}`}
              >
                <div
                  className={`text-neutral-500 shrink-0 font-medium group-hover:text-neutral-400 transition-colors pt-1 ${isPeek ? "w-24 text-xs" : "w-32 text-sm"}`}
                >
                  {col.name}
                </div>
                ...
                {col.type === "select" ? (
                  <div
                    className="relative flex-1 max-w-xs pt-0.5"
                    ref={
                      openSelectId === col.id ? selectDropdownRef : undefined
                    }
                  >
                    <button
                      onClick={() =>
                    ...
                    {openSelectId === col.id && (
                      <div
                        className="absolute z-50 top-full left-0 mt-1 bg-neutral-900 border border-neutral-700 min-w-32 py-1 rounded shadow-xl overflow-hidden"
                        style={{ minWidth: 140 }}
                      >
                        <button
                          onClick={() => {
                            handlePropertyChange(col.id, "");
                            setOpenSelectId(null);
                          }}
                          ...
                          (rawOpt: string | SelectOption) => {
                            ...
                            return (
                              <button
                                key={opt.value}
                                onClick={() => {
                                  handlePropertyChange(col.id, opt.value);
                                  setOpenSelectId(null);
                                }}
                  ...
                  <div className="flex-1 flex flex-col gap-2 pt-0.5">
                        ...
                        {val.map((optVal: string) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  handleMultiSelectToggle(col.id, optVal)
                                }
                    ...
                    {col.options &&
                      (col.options as (string | SelectOption)[]).filter(
                        (o) =>
                          !(
                            Array.isArray(val) &&
                            val.includes(normalizeOption(o).value)
                          ),
                      ).length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                            ...
                            .map((rawOpt) => {
                              ...
                              return (
                                <button
                                  key={opt.value}
                                  onClick={() =>
                                    handleMultiSelectToggle(col.id, opt.value)
                                  }
                  ...
                  >
                    ...
                    >
                      {val ? (
                        <StatusChip value={String(val)} options={col.options} />
                      ) : (
                        <span className="text-neutral-600 text-sm">
                          {tDb("empty")}
                        </span>
                    ...
                    {openSelectId === col.id && (
                      <div
                        className="absolute z-50 top-full left-0 mt-1 bg-neutral-900 border border-neutral-700 py-1 rounded shadow-xl overflow-hidden max-h-72 overflow-y-auto"
                        style={{ minWidth: 192 }}
                      >
                        <button
                          onClick={() => {
                        ...
                        {STATUS_GROUP_ORDER.map((g) => {
                          const groupOpts = (col.options || [])
                            .map(normalizeOption)
                            .filter(
                              (o: SelectOption) => getStatusGroup(o) === g,
                            );
                          if (groupOpts.length === 0) return null;
                          return (
                            <div key={g}>
                              <div className="px-3 pt-1.5 pb-0.5 text-[10px] text-neutral-500 font-semibold uppercase tracking-wider">
                                {statusGroupLabel[g]}
                              </div>
                    ...
                    return (
                      ...
                      >
                        <button
                          onClick={() =>
                            setOpenSelectId(
                              openSelectId === col.id ? null : col.id,
                            )
                          }
                  ...
                  <div className="relative">
                    <button
                      onClick={(e) => {
                        ...
                        setOpenDateColId(
                          openDateColId === col.id ? null : col.id,
                        );
                      }}
                    ...
                    >
                      <span
                        className={
                          val ? "text-neutral-200" : "text-neutral-600"
                        }
                      >
                        {val
                          ? formatDateValue(
                              String(val),
                              col.type as "date" | "datetime",
                              col.dateFormat,
                            )
                          : tDb("empty")}
                      </span>
                  ...
                  <div className="flex items-center gap-1.5 flex-1">
                    <input
                      ...
                      value={val || ""}
                      onChange={(e) =>
                        handleTextPropertyChange(col.id, e.target.value)
                      }
      ...
      {showDeleteConfirm && (
        <ConfirmDialog
          title={t("deleteConfirm", {
            title: properties["title"] || t("untitled"),
          })}
