...
export default async function LandingTools() {
  ...
  return (
    <section id="tools" className="px-14 py-27.5">
      <div className="max-w-7xl mx-auto">
        {/* section header + title */}
        <div className="flex items-end gap-6 mb-9">
          <div>
            <span className="font-mono text-[11px] text-dim uppercase tracking-[0.18em]">
              {t('bridgeToolsSnum')}
            </span>
            <h2
              className="m-0 mt-3.5 mb-1.5 font-sans font-semibold text-neutral-100 leading-[0.98]"
              style={{ fontSize: 40, letterSpacing: '-0.035em' }}
            >
              {t('bridgeToolsH2Part1')}{' '}
              <span className="font-serif italic text-accent-strong" style={{ fontSize: 44 }}>
                {t('bridgeToolsH2Accent')}
              </span>{' '}
              {t('bridgeToolsH2Part2')}
            </h2>
            <p className="m-0 text-[15px] text-dim">{t('bridgeToolsSubhead')}</p>
          </div>
        </div>
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden">
          {/* header row */}
          <div
            className="grid px-4.5 py-3 bg-neutral-850 font-mono text-[11px] text-dim uppercase tracking-[0.08em]"
            style={{ gridTemplateColumns: '90px 1.4fr 1.5fr 0.6fr' }}
          >
          ...
          {TOOLS.map((row, i) => {
            ...
            return (
              <div
                key={i}
                className="grid items-center px-4.5 py-3.25 text-[13.5px]"
                style={{
                  gridTemplateColumns: '90px 1.4fr 1.5fr 0.6fr',
          ...
          {/* footer */}
          <div
            className="flex items-center gap-2 px-4.5 py-3.5 bg-neutral-850 text-[12.5px]"
