...
const TOOLS = [
  ...
  { scope: "write", tool: "create_database", descKey: "create_database" },
  {
    ...
    descKey: "update_database_schema",
  },
] as const;

const RESOURCES = [
  {
    uri: "corpus://workspace/{id}/schema",
    mimeType: "application/json",
    descKey: "bridgeResourcesDescWorkspace",
  },
  {
    uri: "corpus://page/{id}",
    mimeType: "text/markdown",
    descKey: "bridgeResourcesDescPage",
  },
  {
    uri: "corpus://database/{id}/schema",
    mimeType: "application/json",
    descKey: "bridgeResourcesDescDatabase",
  },
  {
    uri: "corpus://audit-log/recent",
    mimeType: "application/json",
    descKey: "bridgeResourcesDescAuditLog",
  },
] as const;
...
const PROMPTS = [
  {
    name: "summarize-page",
    args: "page_id, style?",
    descKey: "bridgePromptsDescSummarizePage",
  },
  {
    name: "weekly-status-report",
    args: "database_id, period?",
    descKey: "bridgePromptsDescWeeklyReport",
  },
  {
    name: "kanban-triage",
    args: "database_id",
    descKey: "bridgePromptsDescKanbanTriage",
  },
  {
    name: "extract-tasks",
    args: "page_id",
    descKey: "bridgePromptsDescExtractTasks",
  },
  {
    name: "search-and-create",
    args: "title, query",
    descKey: "bridgePromptsDescSearchCreate",
  },
] as const;
...
export default async function LandingTools() {
  ...
  const getToolDesc = (toolKey: string) => {
    switch (toolKey) {
      case "search":
        return "Search pages and databases in the workspace by title keyword.";
      case "list_workspace":
        return "List all workspace items (pages & databases). Filter by parentId.";
      case "get_page":
        return "Fetch full content of a page or database row by ID. Auto-detects type.";
      case "get_database_schema":
        return "Get column definitions and select options without fetching rows.";
      case "query_database":
        return "Return schema and rows of a database. Supports property filters.";
      case "create_page":
        return "Create a standalone page or database row with title and content.";
      case "update_page":
        return "Update title, content, or properties of a page. Merges properties safely.";
      case "bulk_update":
        return "Update multiple pages or database rows in a single call.";
      case "delete_page":
        return "Delete a page, database, or row. Requires confirm: true — dry-run by default.";
      case "move_item":
        return "Reparent a sidebar item. Pass null to move to workspace root.";
      case "create_database":
        return "Create a database with a custom column schema. Title column auto-added.";
      case "update_database_schema":
        return "Add or remove columns. Removing requires confirm: true. Title protected.";
      default:
        return "";
    }
  };
  ...
  return (
    <section
      id="tools"
      className="px-4 sm:px-8 lg:px-14 py-16 lg:py-27.5 bg-neutral-950"
    >
      <div className="max-w-7xl mx-auto space-y-14">
        ...
        <div>
          ...
          <div className="mb-8 lg:mb-9">
            ...
            <p className="m-0 text-[14px] lg:text-[15px] text-dim">
              {t("bridgeToolsSubhead")}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="m-0 font-mono text-[11px] text-neutral-100 uppercase tracking-widest font-semibold">
              Protocol Tools{" "}
              <span className="text-dim font-normal">({TOOLS.length})</span>
            </h3>
          </div>
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden">
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-mono text-accent-strong text-[12.5px] text-right">
                      {row.tool.includes("create") ||
                      row.tool.includes("schema") ||
                      row.tool.includes("move")
                        ? "Result"
                        : "Page"}
                    </span>
                  </div>
            ...
            >
              ...
              <Link
                ...
                className="font-mono text-accent-strong text-[12.5px] hover:underline"
              >
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="m-0 font-mono text-[11px] text-neutral-100 uppercase tracking-widest font-semibold">
              Protocol Resources{" "}
              <span className="text-dim font-normal">({RESOURCES.length})</span>
            </h3>
          </div>
            ...
            <div className="overflow-x-auto">
              ...
              {RESOURCES.map((row, i) => {
                ...
                return (
                  ...
                  >
                    <span className="font-mono text-neutral-100 font-medium break-all">
                      {row.uri}
                    </span>
                    <span>
                      <span
                        ...
                        style={{
                          color: mc,
                          background: `color-mix(in oklab, ${mc} 16%, transparent)`,
                        }}
                      >
                        {row.mimeType}
                      </span>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="m-0 font-mono text-[11px] text-neutral-100 uppercase tracking-widest font-semibold">
              Protocol Prompts{" "}
              <span className="text-dim font-normal">({PROMPTS.length})</span>
            </h3>
          </div>
