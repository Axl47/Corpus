...
// needs to connect to the Corpus MCP server. Every builder takes an optional
// `token`:
...
export function buildCursorUrl(mcpUrl: string, token?: string): string {
  ...
  return `cursor://anysphere.cursor-deeplink/mcp/install?name=corpus&config=${btoa(JSON.stringify(config))}`;
}
...
export function buildVscodeUrl(mcpUrl: string, token?: string): string {
  ...
  return `vscode:mcp/install?${encodeURIComponent(JSON.stringify({ name: "corpus", config }))}`;
}
...
export function buildJsonConfig(
  ...
  return JSON.stringify({ mcpServers: { corpus: server } }, null, 2);
}

export const TEST_PROMPT =
  "List all pages and databases in my Corpus workspace";
