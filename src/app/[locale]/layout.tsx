...
export const metadata: Metadata = {
  ...
  description:
    "Corpus is the MCP-Native workspace for vibe coders — kanban boards, databases, and pages that Claude, Cursor, and any AI agent can read and write via MCP.",
  ...
  openGraph: {
    ...
    description:
      "Kanban boards, databases, and pages that Claude, Cursor, and any AI agent can read and write via MCP.",
...
export default async function LocaleLayout({
  ...
  const [headerStore, consentCookieStore] = await Promise.all([
    headers(),
    cookies(),
  ]);
  ...
  const initialConsent = parseConsent(
    consentCookieStore.get(CONSENT_COOKIE)?.value,
  );
  ...
  const activeWorkspaceId = cookieStore.get("corpus_workspace_id")?.value;
  const activeWorkspace =
    workspacesList.find((w) => w.id === activeWorkspaceId) || workspacesList[0];
  const sidebarDensity = (cookieStore.get("corpus_sidebar_density")?.value ??
    "comfortable") as "compact" | "comfortable";
  ...
  const demoBanner =
    session.user.role === "demo" ? (
      <div
        key="demo-banner"
        className="shrink-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-4 px-4 py-2 bg-amber-500/10 border-b border-amber-500/20"
      >
            ...
            <AppShell
              ...
              sidebar={
                <WorkspaceSidebar
                  ...
                  activeWorkspace={
                    activeWorkspace ?? { id: "", name: "Workspace" }
                  }
