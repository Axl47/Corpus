...
export function registerPrompts(server: McpServer, ctx: TokenContext) {
  ...
  server.registerPrompt(
    "summarize-page",
    {
      description: "Summarize a Corpus page or database row.",
      argsSchema: {
        page_id: z
          .string()
          .describe("The workspace item ID or database row ID to summarize"),
        style: z
          .enum(["bullet", "paragraph", "tldr"])
          .optional()
          .default("paragraph")
          .describe("Summary style: bullet list, paragraph, or TL;DR"),
      },
    },
    async ({ page_id, style }) => {
      ...
      const styleInstruction =
        style === "bullet"
          ? "Write the summary as a bullet-point list of key points."
          : style === "tldr"
  ...
  server.registerPrompt(
    "weekly-status-report",
    {
      ...
      argsSchema: {
        database_id: z
          .string()
          .describe("Database ID to generate the report from"),
        period: z
          .string()
          .optional()
          .default("last week")
          .describe('Reporting period, e.g. "last week", "this sprint"'),
      },
    },
        ...
        rows
          .map((row) => {
            ...
            for (const col of schema) {
              ...
              parts.push(
                `${col.name}: ${Array.isArray(val) ? val.join(", ") : String(val)}`,
              );
            }
            return parts.join(" · ");
          })
  ...
  server.registerPrompt(
    ...
    {
      description:
        "Review a kanban board and suggest prioritization, blockers, and next actions.",
      argsSchema: {
        database_id: z
          .string()
          .describe("Database ID of the kanban board to triage"),
      },
    },
    async ({ database_id }) => {
      const result = await queryDatabaseRows(ctx.workspaceId, database_id, 200);
    ...
    "extract-tasks",
    {
      ...
      argsSchema: {
        page_id: z
          .string()
          .describe(
            "The workspace item ID or database row ID to extract tasks from",
          ),
      },
    },
    async ({ page_id }) => {
  ...
  server.registerPrompt(
    ...
    {
      description:
        "Search for similar existing pages and suggest content for a new page to avoid duplication.",
      argsSchema: {
        title: z.string().describe("Title of the page you want to create"),
        query: z
          .string()
          .describe("Search query to find similar existing content"),
      },
    },
    async ({ title, query }) => {
      ...
      const formatted =
        results.length > 0
          ? results
              .map(
                (r: { title: string; id: string; type: string }) =>
                  `- [${r.type}] ${r.title} (id: ${r.id})`,
              )
              .join("\n")
