...
export function registerResources(server: McpServer, ctx: TokenContext) {
  // 1. Workspace Schema — corpus://workspace/{id}/schema
  const workspaceSchemaTemplate = new ResourceTemplate(
    "corpus://workspace/{id}/schema",
    {
      list: async () => ({
        resources: [
          {
            uri: `corpus://workspace/${ctx.workspaceId}/schema`,
            name: "Workspace Schema",
            mimeType: "application/json",
            description:
              "Get the JSON schema of a workspace containing databases and metadata",
          },
        ],
      }),
    },
  ...
  // 2. Page Content — corpus://page/{id}
  const pageTemplate = new ResourceTemplate("corpus://page/{id}", {
    list: async () => {
      ...
      return {
        resources: all.map((p) => ({
          uri: `corpus://page/${p.id}`,
          name: p.title || "Untitled Page",
          mimeType: "text/markdown",
          description:
            "Son güncellenen sayfalar. (Diğer tüm sayfalara doğrudan ID'leri ile corpus://page/{id} üzerinden erişilebilir.)",
        })),
      };
    },
  });
  ...
  server.registerResource(
    ...
    pageTemplate,
    {
      mimeType: "text/markdown",
      description:
        "Get markdown content and properties of a page or database row",
    },
    async (uri, variables) => {
      const page = await getAnyPageById(
        ctx.workspaceId,
  ...
  // 3. Database Schema — corpus://database/{id}/schema
  const databaseSchemaTemplate = new ResourceTemplate(
      ...
      list: async () => {
        ...
        return {
          resources: items
            ...
            .map((dbItem) => ({
              uri: `corpus://database/${dbItem.id}/schema`,
  ...
  // 4. Audit Log — corpus://audit-log/recent
  server.registerResource(
    "Recent Audit Log",
