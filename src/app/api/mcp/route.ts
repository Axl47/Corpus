...
const TOKEN_PREFIX = process.env.MCP_TOKEN_PREFIX ?? "crps";
...
async function verifyBearerToken(
  ...
  if (!match) {
    console.error("[mcp/auth] no_bearer_match", {
      headerPreview: authHeader?.slice(0, 30) ?? null,
    });
    return null;
  }
  const token = match[1];
  ...
  if (parts.length < 3) {
    console.error("[mcp/auth] bad_token_shape", {
  ...
  if (!prefix8 || !secret) {
    console.error("[mcp/auth] empty_prefix_or_secret", {
      scheme,
      hasPrefix: !!prefix8,
      hasSecret: !!secret,
    });
  ...
  if (scheme === "oa") {
    const [row] = await db
      ...
      .where(
        and(
          eq(oauthAccessTokens.tokenPrefix, prefix8),
    ...
    if (!(await bcrypt.compare(secret, row.tokenHash))) {
      console.error("[mcp/auth] oauth_token_hash_mismatch", {
        prefix: prefix8,
      });
      return null;
    }
    if (row.expiresAt.getTime() < Date.now()) {
      console.error("[mcp/auth] oauth_token_expired", {
        prefix: prefix8,
    ...
    console.log("[mcp/auth] oauth_token_ok", {
      prefix: prefix8,
      scope: row.scope,
    });
    return {
  ...
  // Personal access token (crps_ prefix)
  if (scheme !== TOKEN_PREFIX) {
    console.error("[mcp/auth] unknown_scheme", {
      scheme,
      expectedPat: TOKEN_PREFIX,
    });
    return null;
  }
  ...
  const [row] = await db
    ...
    .from(agentTokens)
    .where(
      and(eq(agentTokens.tokenPrefix, prefix8), isNull(agentTokens.revokedAt)),
    )
    .limit(1);
...
async function handleMcpRequest(req: Request): Promise<Response> {
  ...
  const server = new McpServer({ name: "corpus-mcp", version: "1.0.0" });
