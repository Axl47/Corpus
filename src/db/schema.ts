...
export const standalonePages = sqliteTable(
  "standalone_pages",
  {
    id: text("id").primaryKey(),
    itemId: text("item_id")
  ...
  (table) => [index("standalone_pages_item_id_idx").on(table.itemId)],
);
...
export const databases = sqliteTable(
  ...
  (table) => [index("databases_item_id_idx").on(table.itemId)],
);
...
export const pages = sqliteTable(
  ...
  (table) => [index("pages_database_id_idx").on(table.databaseId)],
);
...
// Email invitations for people who don't have a Corpus account yet (or aren't
...
export const sharedPages = sqliteTable(
  "shared_pages",
  {
    ...
    permission: text("permission", { enum: ["read", "write"] })
      .notNull()
      .default("read"),
    width: text("width", { enum: ["narrow", "wide", "full"] })
      .notNull()
      .default("narrow"),
...
export const oauthAccessTokens = sqliteTable(
  ...
  (table) => [
    ...
    index("oauth_access_tokens_refresh_prefix_idx").on(
      table.refreshTokenPrefix,
    ),
  ],
);
